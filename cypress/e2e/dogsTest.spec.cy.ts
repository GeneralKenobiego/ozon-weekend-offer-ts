...
describe("Dogs spec", () => {
  ...
  before(() => {
    ...
    yandexDiscApi.deleteFolder(path).createFolder(path);
  });
    ...
    dogCeoApi.getSubBreeds(randomBreed).then((subBreeds) => {
      if (subBreeds.length) {
        subBreeds.forEach((subBreed, index) => {
          dogCeoApi
            .getSubBreedRandomImageLink(randomBreed, subBreed)
            .then((link) => {
              yandexDiscApi
                .uploadFileViaUrl(`${path}/${subBreed}`, link, 20)
                .getFilesInFolder(path, "created")
                .then((items) => {
        ...
        dogCeoApi.getBreedRandomImageLink(randomBreed).then((link) => {
          yandexDiscApi
            .uploadFileViaUrl(`${path}/${randomBreed}`, link, 20)
            .getFilesInFolder(path, "created")
            .then((items) => {
