...
export class YandexDiscApi {
    ...
    cy.request({
      ...
      if (response.status === 200) {
        cy.request({
          ...
          cy.log(
            `Директория ${path} успешно удалена. Флаг permanently = ${isPermanently}`,
          );
        });
      }
    });
  ...
  private waitOperationStatusInt(
    url: string,
    expectedResult: string,
    counter: number,
    timeoutSec: number,
